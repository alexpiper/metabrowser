...
top_taxa <- function(physeq, taxaRank, numberOfTaxa = 9) {
    ...
    mdf <- reshape2::melt(data = otutab, varnames = c("OTU", "Sample"))
...
ggnorm <- function(physeq, cds, x = "X.SampleID", color = NULL, title = NULL) {
    ...
    countdf <- reshape2::melt(countdf, varnames = c("OTU", "X.SampleID"))
...
ggformat <- function(physeq, taxaRank1 = "Phylum", taxaSet1 = "Proteobacteria",
    ...
    ##
    ## Returns:
    ## - data frame with taxonomic, sample and otu counts informations reshape2::melted together
    ...
    if(!is.null(taxaRank1) & !is.null(taxaSet1)){
        otutab <- otutab[tax_table(physeq)[ , taxaRank1] %in% taxaSet1, , drop = FALSE] #Breaking here
    }
    if (nrow(otutab) == 0) {
...
plot_dist_as_heatmap <- function(dist, order = NULL, title = NULL,
    ...
    ## Returns:
    ## - a ggplot2 object
    data <- reshape2::melt(as(dist, "matrix"))
